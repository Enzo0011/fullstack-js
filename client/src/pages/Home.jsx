      ...
      <div className="container mx-auto h-screen">
        <div className="flex justify-between items-center fixed top-0 left-0 right-0 bg-white p-4 shadow-md">
          ...
          <div className="flex items-center">
            <span className="ml-4">Bienvenue {localStorage.getItem("username")}</span>
            <button type="button" className="ml-4 px-4 py-1 rounded text-white bg-green-500 hover:cursor-pointer hover:opacity-80" onClick={()=>setShowModal(true)}>Créer une room</button>
            <span className="ml-4 font-bold hover:cursor-pointer" onClick={doLogout}>Déconnexion</span>
          </div>
        </div>
